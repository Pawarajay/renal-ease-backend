...
const ensureCanAccessLead = async (req, res, leadId) => {
  ...
  // Allow: assigned to me, created by me, or unassigned
  if (
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role !== "admin") {
        // Show leads assigned to this user, created by this user, OR unassigned
      ...
      if (followUpDue === "today") {
        ...
        whereClause += " AND l.follow_up_date < CURDATE() AND l.status NOT IN ('installation', 'closed')";
      } else if (followUpDue === "upcoming") {
        whereClause +=
          " AND l.follow_up_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 7 DAY)";
      }
...
router.get("/stats", authenticateToken, async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute(
      ...
      SELECT
        COUNT(*) AS totalLeads,
        SUM(CASE WHEN status = 'qualified-lead'  THEN 1 ELSE 0 END) AS newLeads,
        SUM(CASE WHEN status = 'free-inspection' THEN 1 ELSE 0 END) AS qualifiedLeads,   -- free-inspection
        SUM(CASE WHEN status = 'quotation'       THEN 1 ELSE 0 END) AS quotationSentLeads, -- quotation
        SUM(CASE WHEN status = 'installation'    THEN 1 ELSE 0 END) AS convertedLeads,   -- installation
        SUM(CASE WHEN status = 'closed'          THEN 1 ELSE 0 END) AS closedLostLeads,  -- closed
        SUM(CASE WHEN source  = 'booking-engine' THEN 1 ELSE 0 END) AS bookingEngineLeads,
        SUM(CASE WHEN source  = 'whatsapp'       THEN 1 ELSE 0 END) AS whatsappLeads,
        SUM(CASE WHEN source  = 'website'        THEN 1 ELSE 0 END) AS websiteLeads,
        SUM(CASE WHEN source  = 'manual'         THEN 1 ELSE 0 END) AS manualLeads,
        SUM(CASE WHEN follow_up_date IS NOT NULL 
                  AND DATE(follow_up_date) <= CURDATE() 
                  AND status NOT IN ('installation','closed')
             THEN 1 ELSE 0 END) AS pendingFollowUps,
        SUM(
          CASE WHEN status = 'converted'
               THEN COALESCE(estimated_value, 0)
...
router.post(
  ...
  [
    ...
    body("source")
      ...
      .isIn(LEAD_SOURCES)
      .withMessage("Invalid source"),
    // status is NOT validated on POST — new leads always start at "qualified-lead"
    // status changes happen via PATCH using the inline dropdown in the table
    body("priority")
      .optional()
      .isIn(["low", "medium", "high"])
      .withMessage("Invalid priority"),
  ...
  async (req, res) => {
    try {
      ...
      const safeSource    = source    ?? "manual";
      // Always start new leads at "qualified-lead" regardless of what frontend sends
      const safeStatus    = "qualified-lead";
      const safePriority  = priority  ?? "medium";
      const safeEstimatedValue = estimatedValue == null ? 0 : Number(estimatedValue);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Medical CRM: allow conversion from "quotation-sent" or "qualified" statuses too
      const convertibleStatuses = ["free-inspection", "quotation", "installation"];
      if (!convertibleStatuses.includes(lead.status)) {
        return res.status(400).json({
          error: `Lead must be in 'Qualified', 'Quotation Sent' status to convert. Current status: ${lead.status}`,
        });
      ...
      if (existingCustomerByPhone.length > 0) {
        ...
        await pool.execute(
          "UPDATE leads SET status = 'installation', converted_customer_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
          sanitizeParams(existingCustomer.id, id)
        );
